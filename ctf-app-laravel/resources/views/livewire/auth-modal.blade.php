<div x-data="{ isOpen: @entangle('isOpen') }" x-show="isOpen" @keydown.escape.window="isOpen = false" class="fixed inset-0 z-50 flex items-center justify-center">
    <div class="absolute inset-0 bg-black/80 backdrop-blur-sm" @click="isOpen = false"></div>
    <div class="relative bg-gradient-to-br from-[#1a0b2e] to-[#0d0015] border border-[#00ff41]/30 rounded-lg p-8 w-full max-w-md shadow-2xl shadow-[#00ff41]/10">
        <button @click="isOpen = false" class="absolute top-4 right-4 text-gray-400 hover:text-[#00ff41] transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>

        <div class="flex items-center gap-3 mb-6">
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-[#00ff41]"><polyline points="4 17 10 11 4 5"></polyline><line x1="12" y1="19" x2="20" y2="19"></line></svg>
        ...
        <form wire:submit.prevent="handleSubmit" class="space-y-4">
            @if ($mode === 'signup')
                <div>
            ...
            <button type="submit" wire:loading.attr="disabled" class="w-full bg-[#00ff41] text-black font-mono font-bold py-3 rounded hover:bg-[#00ff41]/80 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <div wire:loading.remove wire:target="handleSubmit">
                    @if ($mode === 'login')
                        LOGIN
                    @else
                        REGISTER
        ...
        <div class="mt-6 text-center">
            <button wire:click="switchMode" class="text-gray-400 hover:text-[#00ff41] font-mono text-sm transition-colors">
                @if ($mode === 'login')
