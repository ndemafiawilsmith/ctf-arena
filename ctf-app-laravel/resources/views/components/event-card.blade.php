...
<div
    ...
    <div class="relative h-48 overflow-hidden">
        ...
        @if (isset($event['is_rewarded']) && $event['is_rewarded'])
             <div class="absolute bottom-3 left-3 px-2 py-1 bg-yellow-500 text-black text-xs font-bold rounded flex items-center gap-1 font-mono shadow-md">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="7"></circle><polyline points="8.21 13.89 7 23 12 20 17 23 15.79 13.88"></polyline></svg>
                PRIZES
             </div>
        @endif
    </div>
    ...
    <div class="p-5">
        ...
        <button @if ($isEnded) disabled @endif
            ...
            @else
                ...
                UNLOCK_ACCESS @if($event['price'] > 0) - ${{ number_format($event['price'], 2) }} @endif
            @endif
        </button>
    </div>
</div>
