...
<div class="group relative bg-gradient-to-br from-[#1a0b2e] to-[#0d0015] border border-[#00ff41]/20 rounded-lg overflow-hidden hover:border-[#00ff41]/50 transition-all duration-300 hover:shadow-lg hover:shadow-[#00ff41]/10">
    <div class="relative h-48 overflow-hidden">
        ...
        <!-- Status Badge -->
        <div class="absolute top-3 left-3 px-3 py-1 rounded {{ $statusColor }} {{ $statusTextColor }} font-mono text-xs font-bold flex items-center gap-1">
            @if ($isLive)
        ...
        @if (isset($event['is_paid']) && $event['is_paid'])
            <div class="absolute top-3 right-3 px-3 py-1 rounded bg-purple-600 text-white font-mono text-xs font-bold flex items-center gap-1">
        ...
        @else
            <div class="absolute top-3 right-3 px-3 py-1 rounded bg-[#00ff41]/20 text-[#00ff41] font-mono text-xs font-bold">
                FREE
            </div>
        @endif
    </div>
    ...
    <div class="p-5">
        ...
        <button
            @if($isEnded) disabled @endif
            class="w-full py-3 rounded font-mono font-bold text-sm transition-all flex items-center justify-center gap-2 
            {{ $isEnded 
                ? 'bg-gray-700 text-gray-400 cursor-not-allowed' 
                : (isset($event['has_access']) && $event['has_access'] || (isset($event['is_paid']) && !$event['is_paid']) 
                    ? 'bg-[#00ff41] text-black hover:bg-[#00ff41]/80' 
                    : 'bg-purple-600 text-white hover:bg-purple-700') }}"
        >
            @if ($isEnded)
                EVENT_ENDED
            @elseif (isset($event['has_access']) && $event['has_access'] || (isset($event['is_paid']) && !$event['is_paid']))
